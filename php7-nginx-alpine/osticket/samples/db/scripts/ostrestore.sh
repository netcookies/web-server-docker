...
mysql -h "${dbAddr}" -u "${dbUser}" -p"${dbPassword}" -e "DROP DATABASE ${osticketDatabase}"
echo "Done"
echo

echo "Creating new DB for osticket..."
mysql -h "${dbAddr}" -u "${dbUser}" -p"${dbPassword}" -e "CREATE DATABASE ${osticketDatabase}"
echo "Done"
echo

echo "Restoring backup DB..."
mysql -h "${dbAddr}" -u "${dbUser}" -p"${dbPassword}" "${osticketDatabase}" < "${currentRestoreDir}/${fileNameBackupDb}"
echo "Done"
echo
...
chown -R "${webserverUser}":"${webserverGrp}" "${osticketFileDir}"
chown -R "${webserverUser}":"${webserverGrp}" "${osticketDataDir}"
echo "Done"
echo
